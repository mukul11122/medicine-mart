...
import type { CouponHooks } from "./coupon.hooks"
...
/**
 * Set hooks for Coupon routes
 */
export function setCouponHooks(h: CouponHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["code", "description", "discount", "maxDiscount", "minOrder", "isActive", "validUntil", "usageLimit", "usedCount", "createdAt"] as const
...
/**
 * Create Coupon CRUD routes
 */
export function createCouponRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Coupon not found" } }, 404)
      }
