...
/**
 * Auto-generated CartItem Routes
 *
...
import type { CartItemHooks } from "./cart-item.hooks"
...
/**
 * Set hooks for CartItem routes
 */
export function setCartItemHooks(h: CartItemHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "medicineId", "quantity", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["user", "medicine"] as const
...
/**
 * Create CartItem CRUD routes
 */
export function createCartItemRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[CartItem] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "CartItem not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[CartItem] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[CartItem] Create error:", error)
