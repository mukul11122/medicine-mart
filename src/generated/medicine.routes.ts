...
/**
 * Auto-generated Medicine Routes
 *
...
import type { MedicineHooks } from "./medicine.hooks"
...
/**
 * Set hooks for Medicine routes
 */
export function setMedicineHooks(h: MedicineHooks) {
  hooks = h
}
...
/**
 * Create Medicine CRUD routes
 */
export function createMedicineRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.medicine.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Medicine not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.medicine.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.medicine.update({
        where: { id },
        data: picked.data,
      })
