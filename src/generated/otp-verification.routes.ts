...
/**
 * Auto-generated OtpVerification Routes
 *
...
import type { OtpVerificationHooks } from "./otp-verification.hooks"
...
let hooks: OtpVerificationHooks = {}
...
/**
 * Set hooks for OtpVerification routes
 */
export function setOtpVerificationHooks(h: OtpVerificationHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["phone", "otp", "purpose", "verified", "attempts", "expiresAt", "createdAt"] as const
...
/**
 * Create OtpVerification CRUD routes
 */
export function createOtpVerificationRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.otpVerification.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[OtpVerification] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.otpVerification.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "OtpVerification not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[OtpVerification] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.otpVerification.create({
        data: picked.data,
      })
      ...
      console.error("[OtpVerification] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.otpVerification.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[OtpVerification] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[OtpVerification] Delete error:", error)
