...
import type { OrderItemHooks } from "./order-item.hooks"
...
/**
 * Set hooks for OrderItem routes
 */
export function setOrderItemHooks(h: OrderItemHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["orderId", "medicineId", "quantity", "price", "total"] as const
const RELATION_FIELDS = ["order", "medicine"] as const
...
/**
 * Create OrderItem CRUD routes
 */
export function createOrderItemRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[OrderItem] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "OrderItem not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[OrderItem] Get error:", error)
