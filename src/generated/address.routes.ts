...
import type { AddressHooks } from "./address.hooks"
...
/**
 * Set hooks for Address routes
 */
export function setAddressHooks(h: AddressHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "name", "phone", "line1", "line2", "city", "state", "pincode", "type", "isDefault"] as const
const RELATION_FIELDS = ["user", "orders"] as const
 ...
 */
export function createAddressRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Address not found" } }, 404)
      }
