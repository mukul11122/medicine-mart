...
import type { OrderHooks } from "./order.hooks"
...
/**
 * Set hooks for Order routes
 */
export function setOrderHooks(h: OrderHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["orderNumber", "userId", "addressId", "status", "subtotal", "deliveryCharge", "gstAmount", "discount", "total", "paymentMethod", "paymentStatus", "paymentId", "notes", "prescriptionUrl", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["user", "address", "items"] as const
...
export function createOrderRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Order not found" } }, 404)
      }
